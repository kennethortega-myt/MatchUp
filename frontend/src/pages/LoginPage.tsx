...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (err.response?.status === 403) toast.error('Verifica tu correo antes de iniciar sesión')
      else toast.error(err.response?.data?.detail || 'Credenciales incorrectas')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#070509] text-[#F5F0E8] flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] bg-[#0A0812] border-r border-primary/[0.07] p-12 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 right-0 w-64 h-64 bg-[radial-gradient(ellipse,rgba(124,58,237,0.06),transparent)] pointer-events-none" />
        ...
        <div className="relative">
          <p className="text-[11px] text-primary/70 tracking-[0.22em] uppercase font-bold mb-5">Agara — Conexiones reales</p>
          <blockquote className="text-3xl font-black text-[#F5F0E8]/80 leading-[1.15] tracking-tight mb-5">
            "Cada conexión<br />es una decisión."
          </blockquote>
          <p className="text-sm text-[#F5F0E8]/25 leading-relaxed max-w-xs">
            Privacidad total, perfiles verificados y conexiones que respetan tu tiempo.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-[380px]">
          ...
          <div className="mb-8">
            <h1 className="text-2xl font-black text-[#F5F0E8] tracking-tight mb-1.5">Bienvenido de vuelta</h1>
            <p className="text-sm text-[#F5F0E8]/28">Inicia sesión para continuar</p>
          </div>

          {/* Google */}
          <div className="mb-6 flex justify-center">
            <GoogleLogin onSuccess={handleGoogle} onError={() => toast.error('Error con Google')}
              text="signin_with" shape="pill" theme="filled_black" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Link to="/forgot-password" className="text-[11px] text-primary/60 hover:text-primary transition font-semibold">
          ...
          <p className="text-center text-sm text-[#F5F0E8]/22 mt-7">
            ¿No tienes cuenta?{' '}
            <Link to="/register" className="text-primary hover:text-gold-light font-semibold transition">
              Regístrate gratis
            </Link>
          </p>
        </div>
